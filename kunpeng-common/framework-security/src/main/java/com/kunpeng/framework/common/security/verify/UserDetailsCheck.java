...
import com.kunpeng.framework.common.security.exception.AuthenticationServiceException;
import com.kunpeng.framework.common.util.CommonUtil;
import com.kunpeng.framework.common.util.KPJWTUtil;
import com.kunpeng.framework.modules.dept.mapper.AuthDeptMapper;
import com.kunpeng.framework.modules.dept.po.AuthDeptPO;
import com.kunpeng.framework.modules.menu.mapper.AuthMenuMapper;
import com.kunpeng.framework.modules.menu.po.AuthMenuPO;
import com.kunpeng.framework.modules.project.mapper.AuthProjectMapper;
...
import com.kunpeng.framework.modules.role.mapper.AuthRoleMapper;
...
import com.kunpeng.framework.modules.user.mapper.AuthLoginRecordMapper;
import com.kunpeng.framework.modules.user.mapper.AuthUserDeptMapper;
import com.kunpeng.framework.modules.user.mapper.AuthUserMapper;
import com.kunpeng.framework.modules.user.po.AuthLoginRecordPO;
...
public class UserDetailsCheck implements UserDetailsService {
    ...
    private LoginUserBO common(LoginUserTypeBO loginUserTypeBO, AuthLoginRecordPO loginRecordPO) throws UsernameNotFoundException {
        ...
        if (loginUserBO.getUser().getStatus().equals(LoginUserStatusEnum.LOCK.code())) {
            Date jianSuoTime = CommonUtil.dateAddMinutes(CommonUtil.LocalDateTimeByDate(loginUserBO.getUser().getLockDate()), kunPengUserProperties.getLockTime());
...
//            )
        ...
        CommonUtil.set(RedisSecurityConstant.REDIS_AUTHENTICATION_TOKEN + loginUserTypeBO.getProjectCode() + ":" + loginUserTypeBO.getIdentification(), accessToken, kunPengTokenProperties.getExpireTime());
        CommonUtil.set(RedisSecurityConstant.REDIS_AUTHENTICATION_REFRESHTOKEN + loginUserTypeBO.getProjectCode() + ":" + loginUserTypeBO.getIdentification(), refreshToken, kunPengTokenProperties.getExpireTime() * 10);
        CommonUtil.set(RedisSecurityConstant.REDIS_AUTHENTICATION_LOGINUSER_MESSAGE + loginUserTypeBO.getProjectCode() + ":" + loginUserTypeBO.getIdentification(), CommonUtil.toJsonString(loginUserBO), kunPengTokenProperties.getExpireTime());
    ...
    private String loginError(String userId) {
        ...
        if (!CommonUtil.hasKey(RedisSecurityConstant.LOGIN_ERROR_NUMBER + ":" + authUserPO.getUserName())) {
            ...
            return CommonUtil.format("密码错误,您还有{0}次机会", kunPengUserProperties.getErrorNumber() - errorNumber);
        }
        ...
        if (errorNumber >= kunPengUserProperties.getErrorNumber()) {
