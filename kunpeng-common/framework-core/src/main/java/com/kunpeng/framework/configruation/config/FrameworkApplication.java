...
import com.kunpeng.framework.configruation.properties.KunPengFrameworkAsyncProperties;
import com.kunpeng.framework.configruation.properties.KunPengFrameworkTaskExecutorProperties;
import com.kunpeng.framework.configruation.properties.KunPengFrameworkTaskProperties;
...
public class FrameworkApplication implements AsyncConfigurer {
    ...
    @Autowired
    private KunPengFrameworkTaskExecutorProperties kunPengFrameworkTaskExecutorProperties;
    @Autowired
    private KunPengFrameworkTaskProperties kunPengFrameworkTaskProperties;
    @Autowired
    private KunPengFrameworkAsyncProperties kunPengFrameworkAsyncProperties;
    private Logger logger = LoggerFactory.getLogger(FrameworkApplication.class);

    @PostConstruct
    public void init() {
        KPIconUtil.println(KPIconUtil.PURPLE, Arrays.asList("加载模块 [framework-core] 框架核心模块!", "版本      1.0.0-SNAPSHOT"));
    ...
    **/
    ...
    public TaskScheduler taskScheduler(){
        ...
        //线程池大小
        scheduler.setPoolSize(kunPengFrameworkTaskProperties.getPoolSize());
        //线程名字前缀
    ...
    public ThreadPoolTaskExecutor threadPoolTaskExecutor() {
        ...
        executor.setMaxPoolSize(kunPengFrameworkTaskExecutorProperties.getMaxPoolSize());
        // 设置核心线程数，线程池初始创建的线程数量
        executor.setCorePoolSize(kunPengFrameworkTaskExecutorProperties.getCorePoolSize());
        // 设置任务队列容量，存放待执行任务的队列最大长度
        executor.setQueueCapacity(kunPengFrameworkTaskExecutorProperties.getQueueCapacity());
        // 设置线程空闲时间，超过此时间的空闲线程将被销毁
        executor.setKeepAliveSeconds(kunPengFrameworkTaskExecutorProperties.getKeepAliveSeconds());
    ...
    public Executor getAsyncExecutor() {
        ...
        executor.setCorePoolSize(kunPengFrameworkAsyncProperties.getCorePoolSize());

        // 设置最大线程数：当队列满时，线程池允许创建的最大线程数量
        // 超出核心线程数的线程在空闲时间超过keepAliveSeconds后会被销毁
        executor.setMaxPoolSize(kunPengFrameworkAsyncProperties.getMaxPoolSize());

        // 设置任务队列容量：当核心线程都在忙碌时，新提交的任务会被放入此队列等待执行
        // 队列满后才会创建新线程直到达到最大线程数
        executor.setQueueCapacity(kunPengFrameworkAsyncProperties.getQueueCapacity());
        ...
        executor.setKeepAliveSeconds(kunPengFrameworkAsyncProperties.getKeepAliveSeconds());
