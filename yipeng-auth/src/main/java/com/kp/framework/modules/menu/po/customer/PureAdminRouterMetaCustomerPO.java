...
@NoArgsConstructor
@AllArgsConstructor
@Schema(name = "PureAdminRouterMetaCustomerPO", description = "PureAdminRouterMetaCustomerPO")
public class PureAdminRouterMetaCustomerPO implements Serializable {
    ...
    @Schema(description = "是否缓存该路由页面（开启后，会保存该页面的整体状态，刷新后会清空状态）")
    private Boolean keepAlive;

    @Schema(description = "权限")
    private List<String> auths;

    @Schema(description = "外链内嵌页面的地址")
    ...
    public PureAdminRouterMetaCustomerPO(MenuCustomerPO menu, Map<String, List<MenuPO>> auths) {
        ...
        if (KPStringUtil.isEmpty(this.auths)) {
            ...
            this.auths.addAll(list.stream().map(MenuPO::getPerms).toList());
        }
    }
}
